  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (isActive) {
        const payload = {};
        if (!form.indefinite) {
          payload.deactivate_from = form.deactivate_from;
          payload.deactivate_to = form.deactivate_to;
        }
        await api.post(`/chart-of-accounts/${account.id}/deactivate/`, payload);
      } else {
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        switch (sortConfig.key) {
          ...
          case 'balance':
            ...
            break;
          case 'created_by':
            aValue = (a.created_by_username || '').toLowerCase();
            bValue = (b.created_by_username || '').toLowerCase();
            break;
          case 'created_at':
        ...
        <table style={{ width: "100%", tableLayout: "auto", borderCollapse: "collapse", background: "white" }}>
          ...
          <tbody>
            {filteredAccounts.length === 0 ? (
              <tr>
                <td colSpan={isAdmin ? 9 : 8} style={{ textAlign: "center", padding: 20, borderBottom: "1px solid #ddd" }}>
                  No accounts found.
                </td>
              </tr>
            ) : (
              filteredAccounts.map((account) => (
                <tr key={account.id}>
                  ...
                  <td style={{ 
                    ...
                    fontWeight: "normal",
                    fontSize: "0.85em"
                  }}>{account.created_by_username || 'N/A'}</td>
                  <td style={{ 
                    padding: "10px 12px", 
                    borderBottom: "1px solid #ddd",
                    fontWeight: "normal",
