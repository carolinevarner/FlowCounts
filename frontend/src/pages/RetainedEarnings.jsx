...
export default function RetainedEarnings() {
  ...
  const handleDownload = async () => {
    ...
    const reportCard = document.getElementById('retained-earnings-report-card');
    if (!reportCard) {
    ...
    try {
      ...
      // Save the PDF
      pdf.save(`retained-earnings-${reportDate}.pdf`);
    } catch (error) {
  ...
  if (error) {
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        <div style={{ fontSize: '18px', marginBottom: '10px', color: '#c1121f' }}>Error Loading Retained Earnings Statement</div>
  ...
  return (
    <div style={{ padding: "12px 16px", maxWidth: "100%", boxSizing: "border-box" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            <button
              ...
              style={{
                ...
                borderRadius: "6px",
                padding: "6px 12px",
                cursor: retainedEarningsData ? "pointer" : "not-allowed",
                fontSize: "13px",
                display: "flex",
                alignItems: "center",
                gap: "6px",
                opacity: retainedEarningsData ? 1 : 0.6
              }}
              title="Print report"
            >
      ...
      {retainedEarningsData && (
        <div id="retained-earnings-report-card" className="card" style={{ marginBottom: 20, padding: 0, overflow: "hidden" }}>
          {/* Report Header */}
          <div style={{ 
      ...
      {showEmailModal && retainedEarningsData && (
        <EmailModal
          isOpen={showEmailModal}
          onClose={() => setShowEmailModal(false)}
          recipientType="manager"
          senderRole="ACCOUNTANT"
          initialSubject={`Statement of Retained Earnings - ${dateMode === 'asof' ? new Date(selectedDate).toLocaleDateString() : `${new Date(startDate).toLocaleDateString()} - ${new Date(endDate).toLocaleDateString()}`}`}
          initialMessage={`
...
Add: Net Income: ${formatCurrency(retainedEarningsData.net_income)}
Less: Dividends: ${formatCurrency(0)}
